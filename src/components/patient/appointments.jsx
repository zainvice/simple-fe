...
const Appointments = ( {handleNewAppointmentOpen, loading, handleViewAppointmentOpen, appointments}) => {
  ...
  return (
    ...
    {appointments.filter(appointment=>  appointment.status !== 'Completed' && appointment.status !== 'Cancelled' ).length > 0 ?
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 overflow-y-auto">
       ...
       :
       <div className='flex w-full my-12 text-center'>
      ...
      {appointments.filter(appointment=> appointment.status === 'Cancelled').length > 0 ?
        ...
        :
        <div className='flex w-full my-12 text-center'>
              <p className='font-semibold text-xl text-[#1EBDB8] m-auto'>NO CANCELLED APPOINTMENTS YET</p> 
        </div>
      }
