...
const Sidebar = ({ userType, isExpanded, toggleSidebar, toggleShifter, shifter, showSidebar, showHideSidebar  }) => {
  ...
  return (
    <div 
        className={`h-screen ${
          showSidebar ? "absolute flex w-[70%] translate-x-0 " : "-translate-x-full lg:translate-x-0"
...
const SidebarItem = ({ icon, text, isExpanded, isActive, onClick }) => {
  return (
    <div
      onClick={onClick}
      className={`flex w-[90%] ${isExpanded? '' : 'hidden lg:flex'} rounded-r-[30px] items-center p-5 hover:bg-gray-700 cursor-pointer h-12 my-4 ${isActive ? 'bg-[#1EBDB8]' : ''} transition-all duration-300`}
    >
